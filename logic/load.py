...
def load_graph(file_path):
    ...
    - json: {"nodes": [...], "edges": [...], "directed": true/false}
    - txt: список рёбер (каждая строка: u v [вес])
    - csv: матрица смежности с разделителем-запятой (первая строка — заголовки)
    """
    ...
    if ext == 'json':
        ...
        # Читаем CSV-матрицу смежности с разделителем-запятой
        with open(file_path, 'r', encoding='utf-8') as f:
            reader = csv.reader(f)
            lines = list(reader)
        if not lines:
        ...
        # Первая строка — заголовки (первый элемент пустой или " ")
        headers = lines[0]
        # Если первый элемент пустой (обычно так), пропускаем его
        if headers[0].strip() == '':
            nodes = [h.strip() for h in headers[1:]]
            matrix = []
            for row in lines[1:]:
                if not row or all(c.strip() == '' for c in row):
                    continue
                # первая колонка — имя узла, остальные — числа
                values = [float(x) for x in row[1:]]
                matrix.append(values)
        else:
            # Нет пустого первого элемента — номера строк = номера столбцов
            n = len(headers)
        ...
        # Список рёбер (пробелы/табуляции)
        edges = []
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        for line in lines:
            ...
            parts = line.split()
            if len(parts) < 2:
                continue  # игнорируем строки, где меньше двух полей
            u, v = parts[0], parts[1]
